...
// values: wallLength, wallHeight, blockDepth, segments:[{length,height}], numSegments
export default function WallDiagram({ diagram, values, highlight }) {
  if (diagram === "wallPlan") return <WallPlan values={values} highlight={highlight} />;
  return <WallElevation values={values} highlight={highlight} />;
}

function WallElevation({ values, highlight }) {
  ...
  return (
    <svg viewBox={`0 0 ${VB_W} ${VB_H}`} className="w-full h-auto bg-slate-50 rounded-xl border border-slate-200">
      ...
      {/* Length dim (bottom) */}
      <DimLine x1={x0} y1={baseY + 24} x2={endX} y2={baseY + 24} label={`Length ${fmtLen(len)}`} active={highlight === "wallLength"} />
      ...
      {/* Height dim (left) */}
      <DimLine x1={x0 - 26} y1={baseY} x2={x0 - 26} y2={topY} label={`Ht ${fmtLen(h)}`} active={highlight === "wallHeight"} />
      ...
      {/* Exposed ends markers */}
      <circle cx={x0} cy={baseY} r={4} fill="#0f766e" />
      <circle cx={endX} cy={baseY} r={4} fill="#0f766e" />
...
function WallPlan({ values, highlight }) {
  ...
  // Lay segments end to end turning 90° each (L / U / multiple)
  const pts = [];
  let x = cx - (n * 30), y = cy + 40;
  let ang = 0;
  pts.push([x, y]);
  for (let i = 0; i < n; i++) {
    ...
    <svg viewBox={`0 0 ${VB} ${VB}`} className="w-full h-auto bg-slate-50 rounded-xl border border-slate-200">
      ...
      {pts.map((p, i) => (
        <g key={i}>
          ...
          {i > 0 && (
            <text x={(pts[i - 1][0] + p[0]) / 2} y={(pts[i - 1][1] + p[1]) / 2 - 8} textAnchor="middle" fontSize={10} fill="#0f172a" fontWeight={600}>
